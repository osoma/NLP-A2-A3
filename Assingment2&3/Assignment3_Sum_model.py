...
def summ(x, axis):
  """sum
     ...
     depending on the axis sum will:
       0: compute sum value for a batch and reduce batch size to 1
       1: compute sum value across time slots and reduce time_slots to 1
       2: compute sum value across ndims an reduce dims to 1.
  """
...
model.add(Lambda(lambda x: summ(x,axis=1)))
...
#to save model
pickle.dump(model, open('kerasSumModel.sav', 'wb'))

Y_predict=model.predict(X_test)
Y_testing=Labeling(Y_test)
Y_predicting=Labeling(Y_predict)
print("The Accuracy for LSTM (Sum) model is ",accuracy_score(Y_testing, Y_predicting)*100)
